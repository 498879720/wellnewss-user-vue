    ...
    <ul class="message_ul">
      ...
      <div class="bottom">
         <div class="Input_box">
          ...
          <button id="send"  :disabled="disabledBtn" @click="send">发送</button>
  ...
  export default {
    data() {
      return {
        ...
        messageVal:[],
        pageNum:'1',
        token:'b4xSa/8H03LBHImq8sfmKYbBPnV3owXfereYa80+vD77J+jnTVEyXKkcNXtD1dxozBZKBFFkyJynNYKaYudbghIeLwYGqkT9TvBMDA58Duk=',
        userId:22,
        targetId:'1505877339201',
        newScorll:'0',    //新增控制数据
        listinit:0   //页面初始化完成后
      }
    },
    ...
    created() {
      var _this = this;
      this.$http.post("http://192.168.1.152:8080/healthFront/out/doctors/webDoctorChat.do",this.Qs.stringify({
       // consultId:264,
    ...
    mounted(){
     ...
     // $('<audio id="chatAudio"><source src="/static/img/notify.mp3" type="audio/mp3"></audio>').appendTo('body');
      ...
      var userId = '1505437608866';
      var token = _this.token;


      var msgsss ={type:'0',headImg:'1111',time:'2017-8-16',textimg:'444'};
      //console.log( this.messageVal)
    ...
    'messageVal':{
      ...
      'listinit':{
        handler:function(val,oldVal){
          var _this = this;
          RongIMClient.getInstance().hasRemoteUnreadMessages(this.token,{
            ...
            onReceived: function (message) {
              ...
              switch(message.messageType){
                case RongIMClient.MessageType.TextMessage:
                  // 发送的消息内容将会被打印
                  var inputText = message.content.content;
                  console.log(inputText);
                  var msg = {talkerType:1,consult:{member:{memberImg:'11' } },messageType:0,time:'2017-8-16',content:inputText};
                  _this.messageVal.push(msg);
                  console.log(_this.messageVal)
                  ...
                  //判断接收到的消息是否为方案 true方案
                  //如果消息里有schemeName这个字符串
                  /*     if (inputText.toString().indexOf("schemeName") >= 0){
                         //将消息转为json对象
                         jsonObj = JSON.parse(inputText);
                         var packageScheme = " <li class='Other'>" +
                           "<a href='##'><img class='Head_img' src='${consult.doctors.picture}' alt=''></a>" +
                           "<div class='message_con'>" +
                           " <s><i></i></s>" +
                           "<div class='prescription_con'>" +
                           " <div class='prescription_bg'>" +
                           "<img src='${consult.member.memberImg}' alt=''>" +
                           "<input type='hidden' id='scheme-times' value='"+jsonObj.times+"'>"+
                           "<div class='prescription_text'>" +
                           " <span>"+jsonObj.schemeName+"</span>" +
                           "<span class='see'>点击查看</span>" +
                           "</div>" +
                           "</div>" +
                           " <h5>健康处方</h5>" +
                           ...
                           "</div>" +
                           "</li>";
                         $('.message_ul').append(packageScheme);
                         mlist.push(jsonObj);
                       }else if (isImage(inputText))
                       {
                         var imageLi = "<li class='Other'>" +
                           "<a href='##'><img class='Head_img' src='${consult.doctors.picture}' alt='' ></a>" +
                           "<div class='message_con'>" +
                           "<s><i></i></s>" +
                           "<p><img src='"+inputText+"' alt='' onclick='tu($(this))'></p>" +
                           "</div>" +
                           "</li>";
                         $('.message_ul').append(imageLi);
                       }else {
                         //普通文本
                         $('.message_ul').append(InputDomOfLeft + inputText +InputDom2);
                       }
                       $('.Other').fadeIn(100);
                       $('.text').text('');
                       //Switch();
                       ...
                       var swx = parseFloat(windowHeight)-parseFloat(Bh)-parseFloat(Bhh);

                       if(parseFloat(swx) > parseFloat(Bhh)){
                         $('.new_message').fadeIn(300);
                         $('.new_message').on('click',function(){
                           windowHeight = $('.message_ul').height();
                           $('body').animate({
                             scrollTop: windowHeight
                           },600);
                           $('.new_message').fadeOut(300);
                         });
...
//                 console.log(message.content.content);
      ...
      'loading':{
        handler:function(val,oldVal){
          ...
          if(val == '1'){
              this.$http.post("healthFront/out/doctors/fromMyConsultsToConsult.do",this.Qs.stringify({
                  consultId:264,
      ...
      send:function(){
        ...
        RongIMClient.getInstance().sendMessage(conversationtype, $this.targetId, msg, {
            onSuccess: function (message) {
              //message 为发送的消息对象并且包含服务器返回的消息唯一Id和发送消息时间戳
              console.log("Send successfully");
              var ooos = {talkerType:1,consult:{member:{memberImg:'11' } },messageType:0,time:'2017-8-16',content:thisVal};
              $this.messageVal.push(ooos);
              console.log($this.messageVal)
            },
            onError: function (errorCode,message) {
              var info = '';
